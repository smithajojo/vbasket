...
class ShoppingCart:
    ...
    def AddItem(self, prod):
        cartindex = len(self._cartItems)+1
        c = CartItem(prod,cartindex)
        if len(self._cartItems) == 0:
            ...
            for ci in self._cartItems:
                if prod._title == ci._title:
                    ...
                    ci._price = prod._price
                    ci._total = ci._quantity * ci._price

    def DeleteItem(self,prod):
    ...
    def DisplayCart(self):
        ...
        print ('\033[94m'  + "************************************")
        for ci in self._cartItems:
            print(str(ci._cartIndex) +"> " +ci._title +" X " +str(ci._quantity) +" = " + str(ci._total))
        if len(self._cartItems) == 0:
